...
def save_array_to_rgba_image(np_array: 'np.NDArray', dest_path: str, img_format:str="RGBA", expand_multiplier=(1, 1)):
    ...
    if (dest_rows < base_rows): dest_rows += expand_rows
    if (dest_cols < base_cols): dest_cols += expand_cols
...
def convert_gpx_to_coords(gpx_string: str, silent=True):
    ...
    for m in re.finditer(rx_coord, gpx_string, flags=re.M + re.I):
        ...
        if gps_estimated_meter_distance(last_lat, last_lon, lat, lon) > 500 and i > 0 and not silent:
            raise "Warning: more than 0.5 km!!!"
...
def filter_coords_with_min_distance(coords, min_distance=2.5):
    filtered_coords = []    
    wrotex, wrotey = (0, 0)

    for c in coords:
        if gps_estimated_meter_distance(wrotex, wrotey, c[0], c[1]) >= min_distance:
            filtered_coords.append( (c[0], c[1], 1) )
            (wrotex, wrotey) = c

    return filtered_coords

def simplify_geometry(coords, m: int, n: int, tolerance=2.5):     
    """ Simplify geometry between points m and n """  
    
    def simplify_geometry_recurse(coords, m: int, n: int, tolerance):
        if m == n: return
        ...
        return simplify_geometry_recurse(coords, m, i_max, tolerance) or simplify_geometry_recurse(coords, i_max, n, tolerance)
    ...
    simplify_geometry_recurse(new_coords, m, n, tolerance)
...
def simplify_gpx_route(gpx_string: str, tolerance=2.5):
    ...
    coords = filter_coords_with_min_distance(coords, min_distance=tolerance)
 
    simplified_coords = simplify_geometry(coords, 0, len(coords)-1, tolerance * 4)
